...
function findLastEvenNumber(arr) {
  ...
  if (!Array.isArray(arr)) {
    ...
    return found !== undefined ? found : "Парних чисел не знайдено";
  }
}
...
function sumAndPrint(arr) {
  ...
  const sum = arr.reduce((elements, element) => elements + element, 0);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const arrNew = arr.slice(-n);
  arrNew.forEach((element) => console.log(element));

  const sum = arrNew.reduceRight((el1, el2) => el1 + el2, 0);
  return sum;
}
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((longOne, currentWord) => {
    if (currentWord.length > longOne.length) {
      return currentWord;
    } else {
      return longOne;
    }
  }, ``); // не забути про перший елемент порівняння
...
function capitalizeWords(arr) {
  ...
  //console.log(arr.map((element) => element.at(0).toUpperCase()));
